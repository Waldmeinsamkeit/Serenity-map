...
- AI Context 导出：导出节点、边、选中节点、一跳/二跳上下文、邻接关系和 Mermaid 风格逻辑图。
- AI Patch 导入：支持新增、更新、删除、连接、断开连接、移动节点，并在应用前校验和预览。
...
4. 点击导出 AI Context，查看当前画布的 AI 可读语义上下文。
5. 点击导入 AI Patch，粘贴符合 schema 的 JSON patch，校验通过后应用到画布。
...
Serenity 提供一个本地 `stdio` MCP Server，方便 agent 读取画布语义图、导出 AI Context、校验 AI Patch，并在校验通过后写入本地画布快照。
...
- `serenity_get_context`：返回 AI 可读语义上下文，支持 `json` 和 `markdown`。
- `serenity_get_snapshot`：返回原始本地 tldraw 快照。
- `serenity_validate_patch`：校验 patch，不写入文件。
- `serenity_apply_patch`：校验并应用 patch，原子写入 `store/canvas-default.json`。
